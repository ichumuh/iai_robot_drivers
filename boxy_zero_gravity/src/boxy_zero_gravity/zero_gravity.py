...
class ZeroGravity(object):
    ...
    def joy_cb(self, data):
        ...
        for button_name, button_id in BUTTONS.items():
            buttons_currently_pressed[button_name] = data.axes[button_id] != 0
        if buttons_currently_pressed['L2']:
            if self.buttons_pressed_before['down'] and not buttons_currently_pressed['down']:
                ...
                rospy.loginfo('stiffness of left arm set to {}'.format(self.stiffness['left']))
                ...
                rospy.loginfo('stiffness of right arm set to {}'.format(self.stiffness['right']))
    ...
    def set_min_stiffness(self, arm_name):
        while (self.stiffness[arm_name] > self.min_stiffness).all():
